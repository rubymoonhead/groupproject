...
text/x-generic inc_login_functions.php ( PHP script, ASCII text )
<?php
  ...
  function check_login($connection, $email = '', $pass = '') {
    ...
    if (empty($errors)) {
      // If everything's ok.
      $q = "SELECT artistuser_id, first_name, password, status, role FROM ARTISTUSER WHERE email_address='$e'";
